...
class EmployeeDiscountCommandTest {
    @Test
    void execute_withValidParametersAppendsDiscountAmountToTotalDiscountAmount_shouldPass() {
        ...
        DiscountCommand discountCommand = new EmployeeDiscountCommand(user, products);
        ...
        assertThat(discountCommand.execute(totalDiscountAmount))
                .isEqualTo(
                        ShopUtils.calculateDiscountAmount(ShopUtils.calculateTotalAmount(products).subtract(totalDiscountAmount), DiscountSetting.EMPLOYEE.getDiscountAmount())
                );
